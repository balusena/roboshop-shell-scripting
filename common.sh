...
mongo_schema_setup() {
  ...
  mongo --host mongodb-dev.robobal.store <${app_path}/schema/$component.js  &>>$log_file
  stat_check $?
}
...
mysql_schema_setup() {
  ...
  echo -e "${color} Load Schema ${nocolor}"
  mysql -h mysql-dev.robobal.store -uroot -p${mysql_root_password} </app/schema/${component}.sql  &>>$log_file
  stat_check $?
}
...
go() {
  ...
  echo -e "${color} Download the dependencies & Setup Go Application ${nocolor}"
  cd /app
